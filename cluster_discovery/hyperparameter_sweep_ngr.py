...
if 'data_loading' in parts_to_run:
    ...
    # Get unique indices
    unique_idxs = {k: v for v, k in enumerate(gtin_nos)}
    gtin_nos = list(unique_idxs.keys())
    bias = list([bias[i] for i in unique_idxs.values()])
    vectors = vectors[list(unique_idxs.values())]
